...
class NodeMinibatchIterator(object):
    ...
    def __init__(self, G,  id2idx,
        ...
        self.adj, self.deg = self.construct_adj()
        self.test_adj = self.construct_test_adj()
        self.val_adj = self.construct_val_adj()
    ...
    def construct_val_adj(self):
        adj = len(self.id2idx) * np.ones((len(self.id2idx) + 1, self.max_degree))
        for nodeid in self.G.nodes():
            if self.G.node[nodeid]['val']:
                continue
            neighbors = np.array([self.id2idx[neighbor]
                                  for neighbor in self.G.neighbors(nodeid)])
            if len(neighbors) == 0:
    ...
    def batch_feed_dict(self, batch_nodes, val=False):
        ...
        feed_dict = dict()
        feed_dict.update({self.placeholders['batch_size']: len(batch1)})  # 通过改变placeholder返回出去
        feed_dict.update({self.placeholders['batch']: batch1})  # 要batch的节点索引
    ...
    def next_minibatch_feed_dict(self):   # 这里好像是minibatch的实现
        start_idx = self.batch_num * self.batch_size
        self.batch_num += 1
