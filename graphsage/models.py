...
class GeneralizedModel(Model):  # 相当于没有构建顺序模型的model，必须自己构建顺序模型，算出output，因为重写了build()函数
    """
...
class SampleAndAggregate(GeneralizedModel):
    ...
    def sample(self, inputs, layer_infos, batch_size=None):
        ...
        for k in range(len(layer_infos)):
            t = len(layer_infos) - k - 1  # 相反的顺序
            # if FLAGS.aggregate_with_self :

            support_size *= layer_infos[t].num_samples  # SAGEInfo，namedtuple
            sampler = layer_infos[t].neigh_sampler  # sampler可以调用的对象
            # 调用时其实是UniformNeighborSampler的对象
    ...
    # aggregate()完成了整个网络的聚合操作
    # 论文中提到了，因为minibatch操作，先算出了要采样的节点，所以其实每层邻居采样的个数与layer_info是相反的，比如
    # layer_info 定义的是25 10，那么实际上第一层邻域采的是10个邻居节点，第二层领域采的是25个节点，神经网络的第一层
    # 做的是第一层邻域的聚合，第二层做的事第二层的聚合。
    def aggregate(self, samples, input_features, dims, num_samples, support_sizes,   batch_size=None,
        ...
        """
        # input_features:shape=(2983,16384)
        # dims:[16384,128,128] 应该是feature的长度
        # num_samples:[25,10]
        ...
        # hidden每一个的大小是support_size*minibatch,hidden[0]是要表示的节点
        hidden = [tf.nn.embedding_lookup(input_features, node_samples)  # hidden是一个list，list每一个元素是每层要采样的节点的tensor
                  for node_samples in samples]  # embedding_lookup返回张量对应索引的元素
        new_agg = aggregators is None
        if new_agg:
            aggregators = []

        # choose aggregator model
        #self.aggregator_cls = aggmodel
            ...
            if new_agg:
                ...
                if layer == len(num_samples) - 1:
                    ...
                    aggregator = self.aggregator_cls(dim_mult * dims[layer], dims[layer + 1],
                                                     dropout=self.placeholders['dropout'],
                                                     name=name, concat=concat, model_size=model_size)
                aggregators.append(aggregator)  # aggergator是一个对象，aggregators相当于是一个对象列表
            else:
                aggregator = aggregators[layer]
            # hidden representation at current layer for all support nodes that are various hops away
            next_hidden = []
            # as layer increases, the number of support nodes needed decreases
            # 这个小循环在第一层会计算两次，计算出要表示的节点和它的邻居节点的第一级聚合信息
            # 第二层计算一次，将要表示节点的第一级聚合信息与邻居节点的第一级聚合信息进行聚合
            # 完成两次聚合，得到最终结果
            for hop in range(len(num_samples) - layer):
                dim_mult = 2 if concat and (layer != 0) else 1
                self_dims = [batch_size*support_sizes[hop], dim_mult*dims[layer]]  # 这里先这么写，
                neigh_dims = [batch_size * support_sizes[hop],
                              num_samples[len(num_samples) - hop - 1],  # 这里是相反的，与论文分析一样
                              dim_mult * dims[layer]]
                h = aggregator((tf.reshape(hidden[hop], self_dims),  # self_vecs,hidden是二维张量，
                                tf.reshape(hidden[hop + 1], neigh_dims)))  # neigh_vecs
    ...
    def _build(self):
        labels = tf.reshape(
            ...
            [self.batch_size, 1])
        self.neg_samples, _, _ = (tf.nn.fixed_unigram_candidate_sampler( #这个应该是负采样方法，negative sample
            true_classes=labels,
            num_true=1,
