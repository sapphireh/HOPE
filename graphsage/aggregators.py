...
class GatAggregator(Layer):
    ...
    def __init__(self, input_dim, output_dim, neigh_input_dim=None,
        ...
        with tf.variable_scope(self.name + name + '_vars'):
            ...
            self.vars['self_weights'] = glorot([input_dim, output_dim],
                                               name='self_weights')
            self.vars['hidden_weights'] = glorot([neigh_input_dim, neigh_input_dim],
                                                 name='self_weights')
            if self.bias:
    ...
    def _call(self, inputs):
        ...
        self_attens = []
        neigh_attens = []
        for i in range(int(self.output_dim/gat_out_dim)):
            neigh_red_vecs = tf.layers.dense(neigh_vecs, gat_out_dim)
            self_vecs = tf.expand_dims(self_vecs, axis=1)
            self_red_vecs = tf.layers.dense(self_vecs, gat_out_dim)
            self_vecs = tf.squeeze(self_vecs, axis=1)

            self_f = tf.layers.dense(self_red_vecs, 1)
            neigh_f = tf.layers.dense(neigh_red_vecs, 1)
            logits = tf.tile(self_f, [1, neigh_f.shape[1], 1])+neigh_f
            coefs = tf.nn.softmax(tf.nn.relu(logits), axis=1)
            neigh_atten = tf.matmul(tf.transpose(coefs, [0, 2, 1]), neigh_red_vecs)
            neigh_atten = tf.squeeze(neigh_atten, axis=1)
            self_atten = tf.squeeze(self_red_vecs, axis=1)
            self_attens.append(self_atten)
            neigh_attens.append(neigh_atten)
        from_self = tf.concat(self_attens, axis=-1)
        from_neighs = tf.concat(neigh_attens, axis=-1)
        if not self.concat:
...
class MLPAggregator(Layer):
    ...
    def __init__(self, input_dim, output_dim, neigh_input_dim=None,
                 dropout=0., bias=False, act=tf.nn.relu,
                 name=None, concat=False, **kwargs):
        super(MLPAggregator, self).__init__(**kwargs)
...
class MeanAggregator(Layer):
    ...
    def _call(self, inputs):
        ...
        self_vecs = tf.nn.dropout(self_vecs, 1 - self.dropout)
        #neigh_means = tf.reduce_mean(neigh_vecs, axis=1)
        #neigh_means = tf.reduce_max(neigh_vecs, axis=1)
        ...
        if not self.concat:
            ...
            output = tf.concat([from_self, from_neighs], axis=1)
            #output = tf.concat([from_self, from_self], axis=1)
        # bias
        if self.bias:
            output += self.vars['bias']
        ...
        from_self = tf.matmul(output, self.vars["self_weights"])
        if not self.concat:
            output = tf.add_n([from_self, from_neighs])
        else:
            output = tf.concat([from_self, from_neighs], axis=1)
